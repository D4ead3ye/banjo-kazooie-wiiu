...
| 4 | **PAL dialog parsing** | DONE | All 3 Torch factories (Dialog, QuizQuestion, GruntyQuestion) now handle both US (`01 03 00`) and PAL (`03 07 00` / `03 01 02` / `03 03 00`) headers. PAL dialogs export all 3 language blocks (EN/FR/DE) to the o2r. The port importer reads `langCount` and uses the English block; extra languages are preserved for future multi-language support. |
| 5 | **Asset ID remapping** | DONE | `AssetVersionRemap.h` contains v1.0→v1.1 (934 entries) and v1.0→JP (794 entries) remap tables. `ResourceHelpers.cpp` auto-detects o2r version at boot by entry count and injects the appropriate aliases. PAL uses the v1.1 table (same layout). Zero cost when using a v1.0 o2r. |
...
| 7 | **Multi-language plumbing** | DEFERRED | PAL o2r now contains FR/DE text alongside EN. To enable language switching: importer needs a language index parameter, decomp's PAL language select UI needs to be activated, and the selected language needs to flow through to the importer. The data is all in the o2r — just needs wiring. |
